...
새 대화에서 이어서 작업할 때는 [인수인계 문서](docs/HANDOFF.md)를 먼저 읽으세요. 최신 작업은 2026-09-20 `20260920-motion3`이며 **문법 검사와 자동 테스트 82개를 통과**했습니다. 정지 원본 기준으로 머리·목 비율을 정렬했고 5,376개 조합의 목 연결 검사, 오프라인 320프레임 렌더, 실제 모바일·데스크톱 화면을 확인했습니다. 최종 전달 상태는 인수인계 문서 맨 위에서 확인합니다. 기존 15분 자동 리뷰는 중지 상태를 유지합니다.

휴대폰 화면을 가득 채우는 예식장, 의상별 손인사·춤·박수, 이동 중에만 보이는 좌우·뒷모습을 지원합니다. 꾸벅·폴짝·하트는 제거하고 기존 저장값은 기본 자세로 호환합니다. 이번 수정에서는 충돌 시 매 프레임 걷기·정지가 반복되던 현상을 고치고 예시 하객을 10명으로 줄였습니다. 자동 말풍선은 6~7초 유지하고 같은 하객은 사라진 뒤 8초를 쉽니다. 폭 960px 미만 최대 2개, 그 이상 최대 3개를 순차적으로 표시하며 표시 도중 하객을 교체하지 않습니다. 클릭한 메시지는 7초 유지하며 이때 자동 말풍선은 잠시 쉽니다.
