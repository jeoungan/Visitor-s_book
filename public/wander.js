...
import {walkable,findPath,nearestPoint} from './navigation.js?v=20260920-motion3';
...
export function createWanderer(guest,height){
 ...
 return{...point,anchor:{...point},random,route:[],wait:random()*4,speed:28+random()*16,blocked:0,yielding:false,facing:'front',direction:random()>.5?1:-1,gait:random()*Math.PI*2,moving:false};
}
...
export function stepWanderer(state,dt,height,others=[],stopped=false){
 const previousFacing=state.facing;state.moving=false;state.facing='front';if(stopped)return;
 ...
 // A yielding walker needs enough room for more than one frame before resuming.
 // Otherwise a slower person ahead alternates walking/front portraits at 60 Hz.
 const clearanceStep=state.yielding?Math.min(distance,Math.max(step,8)):step;
 const probe={x:state.x+(target.x-state.x)/distance*clearanceStep,y:state.y+(target.y-state.y)/distance*clearanceStep};
 const blocked=!walkable(next.x,next.y,height)||others.some(other=>{const now=Math.hypot(other.x-state.x,other.y-state.y),after=Math.hypot(other.x-probe.x,other.y-probe.y);return after<34&&after<now-.01});
 if(blocked){
  state.yielding=true;state.wait=.4+state.random()*.35;state.blocked+=state.wait;
  // Keep the first route while giving way; repeated obstruction gets a new one.
  if(state.blocked>=1.1){state.route=[];state.blocked=0}
  return;
 }
 state.blocked=0;state.yielding=false;state.x=next.x;state.y=next.y;state.moving=step>.001;state.facing=movementFacing(dx,dy,previousFacing);state.gait+=step*.16;if(Math.abs(dx)>.02)state.direction=dx<0?-1:1;
}
