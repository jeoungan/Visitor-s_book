...
import {walkable,findPath,nearestPoint} from './navigation.js?v=20260920-poses2';
...
export function createWanderer(guest,height){
 ...
 return{...point,anchor:{...point},random,route:[],wait:random()*4,speed:28+random()*16,blocked:0,facing:'front',direction:random()>.5?1:-1,gait:random()*Math.PI*2,moving:false};
}
...
export function stepWanderer(state,dt,height,others=[],stopped=false){
 ...
 state.blocked=0;state.x=next.x;state.y=next.y;state.moving=step>.001;state.facing=movementFacing(dx,dy,previousFacing);state.gait+=step*.16;if(Math.abs(dx)>.02)state.direction=dx<0?-1:1;
}
