...
test('edits, avatar action and final position persist across reads',async()=>{assert.equal((await request(`/api/guests/${id}`,'PATCH',{...payload,message:'수정된 축하',avatar:{...payload.avatar,action:'heart'}})).status,200);assert.equal((await request(`/api/guests/${id}/position`,'PATCH',{x:820,y:720})).status,200);const g=(await request('/api/guests')).data.guests[0];assert.equal(g.message,'수정된 축하');assert.equal(g.avatar.action,'heart');assert.equal(g.x,820);assert.equal(g.y,720)});
...
test('late action and position requests cannot overwrite later choices',async()=>{
 ...
 for(const [channel,older,newer] of [['action',{action:'wave'},{action:'heart'}],['position',{x:768,y:500},{x:820,y:720}]]){
 ...
 assert.equal(guest.avatar.action,'heart');assert.equal(guest.x,820);assert.equal(guest.y,720);
});
test('profile edits and action requests share the latest action order',async()=>{
 ...
 const edited=await slowPatch(`/api/guests/${id}`,{...payload,avatar:{...payload.avatar,outfit:12,action:'bow'},clientId,version:3},async()=>{assert.equal((await request(`/api/guests/${id}/action`,'PATCH',{action:'heart',clientId,version:4})).status,200)});
 assert.equal(edited.status,200);assert.equal(edited.data.guest.avatar.action,'heart');assert.equal(edited.data.guest.avatar.outfit,12);
});
